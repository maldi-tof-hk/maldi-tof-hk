...
def plot_by_year(
    model: BaseClassifier, metric: str, year_metrics: np.ndarray, path: MetricsPath
):
...
for model in models:
    ...
    for year in years:
        ...
        save_predictions(y_val, y_pred, PredictionPath(model, Phase.BY_YEAR, fold=year))

        evaluate_all_metrics(
            y_val, y_pred, MetricsPath(model, Phase.BY_YEAR, fold=year)
        )
    ...
    plot_by_year(
        model, "Loss", year_metrics[0], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
    plot_by_year(
        model, "Accuracy", year_metrics[1], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
    plot_by_year(
        model, "Precision", year_metrics[2], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
    plot_by_year(
        model, "Recall", year_metrics[3], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
    plot_by_year(
        model, "F1", year_metrics[4], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
    plot_by_year(
        model, "AUPRC", year_metrics[5], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
    plot_by_year(
        model, "AUROC", year_metrics[6], MetricsPath(model, Phase.BY_YEAR, fold=0)
    )
